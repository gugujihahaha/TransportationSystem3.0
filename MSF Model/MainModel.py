...
def processSamples(dataSample):
    # 将 [Batch, Stages, Channels, H, W] 展平为 [Batch*Stages, Channels, H, W]
    return dataSample.contiguous().view(-1, dataSample.shape[2], dataSample.shape[3], dataSample.shape[4])


# --- 修复部分：删除了读取外部权重的代码，改为简单的标准化逻辑 ---
def normlize(data):
    # 简单的标准化：避免 PermissionError
    mean = data.mean()
    std = data.std() + 1e-5
...
def processGlobal(globalfeatures):
    # 这里的 globalfeatures 如果你没做额外特征提取，其实就是图像特征的某种统计，
    # 这里我们做一个简单的 Placeholder，或者你可以根据需要修改
    # 假设我们从 X 中提取简单的全局统计量 [Batch, 36]
    batch_size = globalfeatures.shape[0]
    return torch.zeros(batch_size * OBJECT_K, 36).to(DEVICE)
...
class mergeScales(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        # x1, x2, x3 都是图像输入，这里我们统一使用你生成的特征
        input1 = normlize(processSamples(x1))
        output1 = self.moduleForScale1(input1)

        input2 = normlize(processSamples(x2))
        output2 = self.moduleForScale2(input2)

        input3 = normlize(processSamples(x3))
