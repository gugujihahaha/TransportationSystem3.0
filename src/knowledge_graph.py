...
class TransportationKnowledgeGraph:
    ...
    def _link_roads_to_pois(self, max_distance: float = 100.0):
        ...
        road_nodes = [n for n, d in self.graph.nodes(data=True) if d.get('type') == 'road_node']
        
        for poi_node in poi_nodes:
            poi_data = self.graph.nodes[poi_node]
            poi_lat = poi_data['latitude']
            poi_lon = poi_data['longitude']
            ...
            for road_node in road_nodes:
                road_data = self.graph.nodes[road_node]
                road_lat = road_data['latitude']
                road_lon = road_data['longitude']
