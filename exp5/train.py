"""
训练脚本 (Exp5 - 数据清洗 + 弱监督上下文增强)
在 Exp4 基础上添加两阶段数据清洗和弱监督上下文处理

核心改进:
1. 第一阶段清洗：基础处理（固定序列长度、缺失特征填充）
2. 第二阶段清洗：深度清洗（异常值剔除、轨迹连续性处理、方向修正）
3. 弱监督上下文：OSM/天气仅用于表示层，不影响决策层
4. 清洗统计：输出清洗前后的样本数量统计
"""
import os
import sys
import argparse
...
# 尝试导入 common 模块（可选）
try:
    from common import BaseGeoLifePreprocessor, Exp5DataAdapter
    HAS_COMMON = True
except ImportError:
    HAS_COMMON = False
...
META_CACHE_PATH = os.path.join(CACHE_DIR, 'cache_meta_cleaning.json')
...
def validate_cache(osm_path: str, weather_path: str) -> bool:
    ...
    try:
        ...
        if meta.get('experiment') != 'exp5_cleaning':
            print(f"⚠️ 缓存实验类型不匹配")
            return False
...
def load_data(geolife_root: str, osm_path: str, weather_path: str,
    ...
    if use_base_data and HAS_COMMON and os.path.exists(BASE_DATA_PATH):
        print(f"\n{'='*80}")
        print("阶段 3: 使用预处理的基础数据（快速模式 - 含第二阶段清洗）")
        print(f"{'='*80}\n")

        try:
            base_segments = BaseGeoLifePreprocessor.load_from_cache(BASE_DATA_PATH)
            adapter = Exp5DataAdapter(target_length=FIXED_SEQUENCE_LENGTH, enable_cleaning=True)
    ...
    try:
        ...
        save_cache_metadata(osm_path, weather_path, geolife_root,
                           len(all_features_and_labels), label_encoder, cleaning_stats)
    except Exception as e:
        print(f"⚠️ 缓存保存失败: {e}")
...
def main():
    ...
    if PYCHARM_MODE:
        ...
        parser = argparse.ArgumentParser(description='训练交通方式识别模型 (Exp5 - 数据清洗 + 弱监督上下文)')
    ...
    print("\n" + "=" * 80)
    print("Exp5 交通方式识别训练 (数据清洗 + 弱监督上下文增强)")
    print("=" * 80)
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(checkpoint, os.path.join(args.save_dir, 'exp5_model.pth'))
            print("✓ 保存最佳模型（基于验证集）")
        else:
            epochs_no_improve += 1
    ...
    print(f"模型已保存到: {os.path.join(args.save_dir, 'exp5_model.pth')}")
