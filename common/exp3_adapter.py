...
class Exp3DataAdapter:
    ...
    def load_cleaned_data(self) -> List[Tuple[np.ndarray, np.ndarray, str]]:
        """
        加载清洗后的数据并生成增强KG特征（推荐方式）

        Returns:
            处理后的数据列表 [(trajectory, enhanced_kg_features, label), ...]
        """
        if not self.use_cleaned_data:
        ...
        print(f"\n{'=' * 80}")
        print(f"Exp3 数据适配 - 加载清洗后数据 + 生成增强KG特征")
        print(f"{'=' * 80}\n")
        ...
        # 处理清洗后的数据（轨迹 + 增强KG特征）
        features = []
        for seg in tqdm(cleaned_segments, desc="[生成增强KG特征]"):
            # 提取清洗后的轨迹
            ...
            features.append((trajectory, enhanced_kg_features, label))

        print(f"\n✅ 处理完成: {len(features)} 个样本")
        print(f"   - 轨迹特征: {trajectory.shape}")
        print(f"   - 增强KG特征: {enhanced_kg_features.shape}")
    ...
    def process_segments(self, base_segments: List[dict]) -> List[Tuple[np.ndarray, str]]:
        ...
        """
        print(f"\n{'=' * 80}")
        print(f"Exp3 数据适配 - 两阶段清洗 (模式: {self.cleaning_mode})")
        print(f"{'=' * 80}\n")
