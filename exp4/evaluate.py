...
from train import TrajectoryDatasetForCCA
...
def main():
    ...
    CACHE_PATH  = 'cache/processed_features_exp4.pkl'
    OUTPUT_DIR  = 'evaluation_results'
    DEVICE      = 'cuda' if torch.cuda.is_available() else 'cpu'

    print("\n" + "=" * 60)
    print("Exp4 模型评估 (轨迹+空间+CCA)")
    print("=" * 60)
    print(f"设备: {DEVICE}")
    ...
    print(f"     - 段级统计维度: {config['segment_stats_dim']}")
    print(f"     - 类别数:   {config['num_classes']}")
    print(f"     - 类别:     {list(class_names)}")
    print(f"     - CCA 损失权重: {config.get('context_loss_weight', 0.1)}")
    print(f"     - 温度参数: {config.get('temperature', 0.07)}")

    model = CCATransportationClassifier(
        trajectory_feature_dim=config['trajectory_feature_dim'],
        segment_stats_dim=config['segment_stats_dim'],
        hidden_dim=config['hidden_dim'],
        num_layers=config['num_layers'],
        num_classes=config['num_classes'],
        dropout=config.get('dropout', 0.3),
        context_loss_weight=config.get('context_loss_weight', 0.1),
    ...
    dataset = TrajectoryDatasetForCCA(
        all_features,
    ...
    with torch.no_grad():
        for traj, stats, labels in tqdm(test_loader, desc="Evaluation Progress"):
            traj    = traj.to(DEVICE)
    ...
    try:
        ...
        plt.title('Exp4 Confusion Matrix (Trajectory + Spatial + CCA)', fontsize=14)
