...
from src.model_full import TransportationModeClassifierFull
...
class TrajectoryDatasetExp4(Dataset):
    ...
    def __getitem__(self, idx):
        traj, spatial, weather, stats, label = self.data[idx]
        ...
        if self.spatial_mean is not None:
            spatial = (spatial - self.spatial_mean) / (self.spatial_std + 1e-8)
...
def load_data(osm_path):
    """从cleaned_balanced.pkl加载数据，提取9维轨迹+12维空间+7维天气特征"""
    ...
    # 提取特征
    print(f"\n[特征提取] 提取9维轨迹+12维空间+7维天气特征...")
    feature_extractor = FeatureExtractor(spatial_extractor)
    all_features = []
    for traj, stats, datetime_series, label in tqdm(cleaned_data, desc="[特征提取]"):
        try:
            # 提取轨迹和空间特征
            traj_9, spatial_12 = feature_extractor.extract_features(traj)
            # 提取天气特征
            weather_7 = weather_processor.extract_weather_features(datetime_series)
            all_features.append((traj_9, spatial_12, stats, label, weather_7))
        except Exception:
            continue

    # 编码标签
    all_labels_str = [item[3] for item in all_features]
    label_encoder = LabelEncoder().fit(all_labels_str)
    all_features = [(traj, spatial, stats, label_encoder.transform([label])[0], weather)
                    for traj, spatial, stats, label, weather in all_features]

    # 过滤NaN和Inf
    all_features = [
        (traj, spatial, weather, stats, label)
        for traj, spatial, stats, label, weather in all_features
        if not (np.isnan(traj).any() or np.isinf(traj).any() or
                np.isnan(spatial).any() or np.isinf(spatial).any() or
...
def main():
    ...
    # 1. 加载数据
    all_features, label_encoder = load_data(args.osm_path)
    if not all_features:
        return
    ...
    labels_encoded = [item[4] for item in all_features]
    ...
    spatial_all = np.vstack([s[1] for s in train_data])
    weather_all = np.vstack([s[2] for s in train_data])
    stats_all   = np.vstack([s[3] for s in train_data])
    ...
    spatial_mean, spatial_std = safe_stats(spatial_all)
    ...
    # 5. 模型
    model = TransportationModeClassifierFull(
        TRAJECTORY_FEATURE_DIM, SPATIAL_FEATURE_DIM, WEATHER_FEATURE_DIM, 18,
        args.hidden_dim, args.num_layers, len(label_encoder.classes_), args.dropout,
        num_segments=5, local_hidden=64, global_hidden=128,
    ).to(args.device)

    class_weights = compute_class_weights(
        label_encoder, train_data, label_index=4, mode='sqrt_inverse'
    )
