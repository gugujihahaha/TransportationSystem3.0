...
class TransportationModeClassifierWithWeather(nn.Module):
    ...
    def __init__(self,
        ...
        self.weather_lstm = nn.LSTM(
            ...
            hidden_size=hidden_dim // 4,  # 较小的隐藏层
            num_layers=num_layers,
            batch_first=True,
            dropout=dropout if num_layers > 1 else 0,
            bidirectional=True
        ...
        # KG: 128 (64*2)
        # 天气: 64 (32*2)
        fusion_input_dim = hidden_dim * 2 + hidden_dim + hidden_dim // 2
    ...
    def forward(self, trajectory_features: torch.Tensor,
        ...
        trajectory_repr = trajectory_out[:, -1, :]  # (batch, 256)

        # KG特征编码
        kg_out, _ = self.kg_lstm(kg_features)
        kg_repr = kg_out[:, -1, :]  # (batch, 128)

        # 天气特征编码（新增）
        weather_out, _ = self.weather_lstm(weather_features)
        weather_repr = weather_out[:, -1, :]  # (batch, 64)
    ...
    def predict(self, trajectory_features: torch.Tensor,
        ...
        self.eval()
        with torch.no_grad():
            logits = self.forward(trajectory_features, kg_features, weather_features)
